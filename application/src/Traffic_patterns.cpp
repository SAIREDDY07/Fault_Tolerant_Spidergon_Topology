...
void traffic_patterns::fixRanges(int &destx , int &desty, int  &destz, int num_rows,int num_cols, int slice_size)
{
    // Fix ranges
    if (destx<0) destx=0;
    if (desty<0) desty=0;
    ...
    if (destz>=slice_size) destz=slice_size-1;	
}
...
int traffic_patterns::trafficTranspose(int tileID,int num_rows, int num_cols)
{
    ...
    desty=srcx;
    destz=destx;	
    fixRanges(destx, desty, destz,num_rows,num_cols, slice_size);
    dnode = desty+destx*num_cols;
    return dnode;
}


int traffic_patterns::trafficTransposez(int tileID,int num_rows, int num_cols, int slice_size)
{
    int srcx;
    int srcy;
    int srcz;
