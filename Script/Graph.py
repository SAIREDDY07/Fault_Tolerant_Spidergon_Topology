...
RT_ALGO = ['WSF', 'NUL', 'XYZ', 'ZXY']
#DATA_PATTERN = ['Bursty', 'CBR']
DATA_PATTERN =['CBR']
TRAFFIC = ['RANDOM', 'TRANSPOSE']
LOAD = ['10', '100', '10']
PATH = os.path.abspath('../nirgam-3.0/')
SOURCES = []
ROWS = 4
COLS = 4
SLICES = 2
...
def generateGraph():
	global RT_ALGO, TRAFFIC, LOAD, PATH, SOURCES, ROWS, COLS, SLICES, DATA_PATTERN
	dataFilePath = os.path.abspath('../Results_'+ 'dim_' +str(ROWS)+str(COLS)+str(SLICES))
	print dataFilePath
	GraphPath = os.path.abspath('../Graphs/Results'+ 'dim_' +str(ROWS)+str(COLS)+str(SLICES))
	print dataFilePath
	dirs = os.listdir('..')
	if 'Results'+'_dim_'+str(ROWS)+str(COLS)+str(SLICES) not in dirs:
	...
	for traffic in TRAFFIC:
		for pattern in DATA_PATTERN:
			...
			try:
				...
				plt.savefig(GraphPath + '/' + traffic + '-' + pattern + '-WSF-NUL.png')
				plt.close()
			except:
				print '\nGraph Error'
