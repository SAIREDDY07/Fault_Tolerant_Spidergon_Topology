...
RT_ALGO = ['XYZ','ZXY','PROM3D']
DATA_PATTERN = ['Bursty', 'CBR']
TRAFFIC = ['TRANSPOSE', 'RANDOM']
LOAD = ['10', '100', '10']
PATH = os.path.abspath('../nirgam-3.02/')
SOURCES = []
ROWS = 3
COLS = 3
SLICES = 4
...
def changeRoutingAlgo(algo, nirgam):
	global ROWS, COLS, SLICES
	nirgamFileHandle = open(nirgam + '/config/nirgam.config', 'w')
	tags = ['TOPOLOGY', 'NUM_ROWS', 'NUM_COLS', 'NUM_SLICE', 'LOG', 'RT_ALGO', 'WARMUP', 'DIRNAME', 'SIM_NUM', 'TG_NUM', 'NUM_BUFS']
	value = ['MESH3D', '', '', '', '4', '', '10', 'Results', '5000', '3000', '10']
	i = 0
	for tag in tags:
		if tag == 'NUM_ROWS':
			value[i] = str(ROWS)
...
def changeSources(pattern, nirgam):
	global ROWS, COLS, SLICES, SOURCES
	AppFileHandle = open(nirgam + '/config/application.config', 'w')
	if not SOURCES:
		for tile in range(0, ROWS*COLS*SLICES):
...
def runSimulation():
	...
	dataFilePath = os.path.abspath('../Results'+str(ROWS)+str(COLS)+str(SLICES))
	nirgamPath = PATH
	simulationResultFilePath = nirgamPath + '/results/Results/stats/sim_results'
	dirs = os.listdir('..')
	if 'Results'+str(ROWS)+str(COLS)+str(SLICES) not in dirs:
		...
		for pattern in DATA_PATTERN:
			for algo in RT_ALGO:
				...
				for load in range(int(LOAD[0]), int(LOAD[1]) + 1, int(LOAD[2])):
					...
					print 'Running simulation for', traffic, 'traffic with ', pattern, 'pattern, ', algo, 'algorithm at load value', load, '...'
					for i in range(0, 10):
						print '\tfor', i + 1, 'time...'
...
#						print output
					...
					avgLatency = avgLatency/i
					avgThroughput = avgThroughput/i
					print '\tAvg Latency =', avgLatency, '; Avg Throughput =', avgThroughput
