    ...
    for topo, pattern_data in results.items():
        for pattern, algos in pattern_data.items():
            ...
            for algo, data in algos.items():
                pir = data.get("PIR", [])
                latency = data.get("Global_Latency", [])
                if pir and latency:
                    plt.plot(pir, latency, marker='o', label=f'{algo} - Latency')
            ...
            plt.legend()
            plt.grid(True)
            plt.xscale('line')  # Set x-axis scale to linear

            # Throughput vs PIR
            plt.subplot(1, 3, 2)
            for algo, data in algos.items():
                pir = data.get("PIR", [])
                throughput = data.get("Global_ThroughPut", [])
                if pir and throughput:
                    plt.plot(pir, throughput, marker='o', label=f'{algo} - Throughput')

            plt.title(f'Throughput vs PIR for {pattern} in {topo}')
            plt.xlabel('PIR')
            ...
            for algo, data in algos.items():
                ...
                if pir and power:
                    plt.plot(pir, power, marker='o', label=f'{algo} - Power')

            plt.title(f'Power vs PIR for {pattern} in {topo}')
            plt.xlabel('PIR')
            plt.ylabel('Power (W)')
            plt.legend()
            plt.grid(True)
