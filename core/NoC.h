...
struct signals {
	...
	sc_signal<flit> sig_toF;			///< data line (flit line) from a tile to its Front neighbor in 3D topology
	sc_signal<flit> sig_fromF;			///< data line (flit line) to a tile from its Front neighbor in 3D topology
	...
	sc_signal<creditLine> cr_sig_toF[NUM_VCS];	///< credit line (transmits buffer status) per virtual channel from a tile to its Front neighbor 
	sc_signal<creditLine> cr_sig_fromF[NUM_VCS];	///< credit line (transmits buffer status) per virtual channel to a tile from its Front neighbor
...
//////ESTIMATE SIGNALS//////////
	...
	sc_signal<flit> est_sig_fromF;			///< data line (flit line) to a tile from its Front neighbor //CODELINE
	sc_signal<flit> est_sig_toF;			///< data line (flit line) from a tile to its Front neighbor //CODELINE
...
/*added*/
	...
	sc_signal<powerCreditLine> power_sig_fromF;			///< data line (flit line) to a tile from its Front neighbor //CODELINE
	sc_signal<powerCreditLine> power_sig_toF;			///< data line (flit line) from a tile to its Front neighbor //CODELINE
...
struct temp_signals {
	...
	sc_signal<flit>	tempsig_toF;
	sc_signal<flit>	tempsig_fromF;	
	};
...
struct NoC : public sc_module {
	...
	/// Constructor
	SC_HAS_PROCESS(NoC);
	NoC(sc_module_name NoC, int num_rows, int num_cols, int num_slice);
	
	int rows;	///< number of rows in topology
	int cols;	///< number of columns in topology
	int slice;	///< number of slices in 3DMESH topology	
	
	BaseNWTile	*nwtile3D[MAX_NUM_SLICE][MAX_NUM_ROWS][MAX_NUM_COLS];	///< A 3-D array of network tiles
	signals		sigs3D[MAX_NUM_SLICE][MAX_NUM_ROWS][MAX_NUM_COLS];	///< Signals to interconnect network tiles
