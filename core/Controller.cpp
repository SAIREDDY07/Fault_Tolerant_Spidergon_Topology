...
Controller<num_ip>::Controller(sc_module_name Controller): sc_module(Controller)
{
    ...
    case QRT:
        ...
        break;
    case CRT:
        libname = libname + string("C_router.so");
        break;
        //DYAD//
   case DYAD:
   ...
   case FTXY:
        ...
        break;
   case NL:
	libname=libname+ string("NL_router.so");
	break;
   case NG:
	libname=libname+ string("NG_router.so");
	break;
   case WF:
	libname = libname + string("WF_router.so");
	break;
   case FTNL:
	libname=libname+ string("FTNL_router.so");
	break;
   case FTNG:
	libname=libname+ string("FTNG_router.so");
	break;
   case FTWF:
	libname = libname + string("FTWF_router.so");
	break;
   case PROM:
	libname=libname+ string("PROM2D_router.so");
	break;
   case FTPROM:
	libname=libname+ string("FTPROM_router.so");
	break;
   case FTPROM3D:
	libname=libname+ string("FTPROM3D_router.so");
	break;
   case FTOE:
	libname=libname+ string("FTOE_router.so");
	break;	
        //ERA
    case ER:
        ...
        break;
    case XYZ:
	libname = libname + string("XYZ_router.so");
	break;
    case ZXY:
	libname = libname + string("ZXY_router.so");
	break;
    case PROM3D:
	libname = libname + string("PROM3D_router.so");
	break;

     case WSF:
	libname = libname + string("WSF_router.so");
	break;
     case NUL:
	libname = libname + string("NUL_router.so");
	break;
     case FTNUL:
	libname = libname + string("FTNUL_router.so");
	break;	
     case FTNF3D:
	libname = libname + string("FTNF3D_router.so");
	break;
     case FTWSF:
	libname = libname + string("FTWSF_router.so");
	break;
     case FTXYZ:
	libname = libname + string("FTXYZ_router.so");
	break;
     case OE3D:
	libname = libname + string("OddEven3D_router.so");
	break;
...
struct congestion cong_status;
				cong_status.DNorth=getvcstatus(N);		
				cong_status.DSouth=getvcstatus(S);		
				cong_status.DWest=getvcstatus(W);		
				cong_status.DEast=getvcstatus(E);		
	
				UI op_dir = rtable->calc_next(temp, src, dest,cong_status);
...
template<UI num_ip>
UI Controller<num_ip>::getvcstatus(int dir)
{
	int i;
	bool x;
	for(i=0;i<NUM_VCS;i++)
	{
		x=Icredit[dir][i].read().freeVC;
		if(x==true)
			return i;
	}
	return NUM_VCS;
...
///////////////////////////////////////////////////////////////////////////
/// Method to assign tile IDs and port IDs of 3D topology
///////////////////////////////////////////////////////////////////////////
template<UI num_ip>
void Controller<num_ip>::setTileID(UI id, UI port_N, UI port_S, UI port_E, UI port_W, UI port_F, UI port_R) {
	tileID = id;
	portN = port_N;
	portS = port_S;
	portE = port_E;
...
/////////////////////////////////////////////////////////////////////
/// Returns direction (N, S, E, W, F, R) corresponding to a given port id
////////////////////////////////////////////////////////////////////
template<UI num_ip>
UI Controller<num_ip>::idToDir(UI port_id) {
	//cout<<" port_id="<<port_id;
	//cout<<" portN="<<portN<<" portS="<<portS<<" portE="<<portE<<" portW="<<portW<<" portF="<<portF<<" portR="<<portR;
	if(port_id == portN) //{cout<<" port_idN="<<portN;
		return N;	//		}
	else if(port_id == portS)	//{cout<<" port_idS="<<portS;
		return S;		//	}
	else if(port_id == portE)	//{cout<<" port_idE="<<portE;
		return E;		//	}
	else if(port_id == portW)	//{cout<<" port_idW="<<portW;
		return W;		//	}
	else if(port_id == portF)	//{cout<<" port_idF="<<portF;
		return F;		//	}
	else if(port_id == portR)	//{cout<<" port_idR="<<portR;
		return R;		//	}
	else				//{cout<<" port_idC="<<C;
		return C;		//}
}
...
template struct Controller<NUM3D_IC>;
template struct Controller<NUM3D_IC_SB>;
