...
//////////////////////////////////////////////////////////////////////////
template<UI num_ip = NUM_IC_B>
//template<UI num_ip = NUM3D_IC>
...
/*	PENALTY	pQ[100];
	...
	void setTileID(UI tileID, UI portN, UI portS, UI portE, UI portW, UI portF, UI portR);
	
	void setTileID(UI tileID, UI portN, UI portS, UI portE, UI portW);
	void allocate_route();	///< does routing
	UI idToDir(UI);		///< returns port id for a given direction (N, S, E, W, F, R)
	UI dirToId(UI);		///< returns direction (N, S, E, W, F, R) corresponding to a given port id
		// PROCESSES END /////////////////////////////////////////////////////////////////////////
	...
	UI portF;	///< port number representing FRONT direction
	UI portR;	///< port number representing REAR direction

	/*added*/	
	double P;//register to store power dissipated at the tile
	double Pnb[6];//registers to store the credit info of power from neighbouring nodes
	/*end*/
	ULL sim_count;
