    ...
    while (true)
    {
        ...
        for (UI i = 0; i < num_ip; i++)
        {
            if (vcRequest[i].event() && vcRequest[i].read() == true)
            {
                ...
                sc_uint<3> dir = opRequest[i].read();
...
///////////////////////////////////////////////////////////////////////////
template<UI num_ip>
void VCAllocator<num_ip>::setTileID(UI id, UI port_N, UI port_S, UI port_E, UI port_W, UI port_F, UI port_R){
	tileID = id;
	portN = port_N;
	portS = port_S;
	portE = port_E;
...
template struct VCAllocator<NUM3D_IC>;
template struct VCAllocator<NUM3D_IC_SB>;
