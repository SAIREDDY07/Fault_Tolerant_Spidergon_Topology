...
void InputChannel<num_op> :: route_flit()
{
    ...
    while (sim_count < SIM_NUM)
    {
        ...
        if (switch_cntrl.event())
        {
            ...
            if (vc[vc_to_serve].vc_route == ND) //5)  	// route not set, require routing
            {
    ...
    while (sim_count < SIM_NUM)
    {
        ...
        if (switch_cntrl.event())  	// clock event
        {
            ...
            */
            ...
            if (vc[vc_to_serve].vc_route == ND)  //5)	// routing decision pending, before transmission
            {
                //if (tileID==0)
		...
		case SPIDERGON:
            ...
            case MESH3D:
				...
				case W: i = portW;
					break;
				case F: i = portF; //cout<<" i= portF="<<portF;
					break;
				case R: i = portR; //cout<<" i= portR="<<portR;
					break;
				case C: i = num_op - 1; //cout<<" i= portC="<<num_op-1;
					break;
				default:
					//cout<<"\n ERROR  InputChannel.cpp transmit_flit()   "; 
					break;
                	}
	       		if (i == ND)
		        {
		            cout<<this->name()<<endl;
		            cout<<"dir = "<<dir<<"	i = ND = 7"<<endl;
...
///////////////////////////////////////////////////////////////////////////
template<UI num_op>
void InputChannel<num_op>::setTileID(UI id, UI port_N, UI port_S, UI port_E, UI port_W, UI port_F, UI port_R) {
//cout<<"\n 3D SetTileID="<<id<<" portN="<<portN<<" portS="<<portS<<" portE="<<portE<<" portW="<<portW<<" portF="<<portF<<" portR="<<portR;
	tileID = id;
	portN = port_N;
	portS = port_S;
	portE = port_E;
...
template struct InputChannel<NUM3D_OC>;
template struct InputChannel<NUM3D_OC_SB>;
