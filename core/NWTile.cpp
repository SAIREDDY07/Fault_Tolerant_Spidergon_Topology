...
//		ip = NULL; modified
        ...
        ip = ((ipcore*(*)())(mkr))();
        cout<<"Attaching application Sink.so";
    }
    else
    {
        void *hndl;
        ...
        mkr = dlsym(hndl, "maker");
        ip = ((ipcore*(*)())(mkr))();
        cout<<"Attaching application "<<app_libname[id];
    }
	...
	case SPIDERGON: 
        ...
        case MESH:
         ...
         case MESH3D:
			...
			{	
				Ichannel[i]->cntrlID = F;			}
			else if(i == portR)
			{	
				Ichannel[i]->cntrlID = R;			}
			else if(i == num_ic - 1)
			{	
				Ichannel[i]->cntrlID = C;			}
			break;
	...
	case SPIDERGON: 
        ...
        case MESH3D:
			if(i == portN)
				...
				Ochannel[i]->cntrlID = F;
			else if(i == portR)
				Ochannel[i]->cntrlID = R;
			else if(i == num_oc - 1)
				Ochannel[i]->cntrlID = C;
			break;			
        }
...
void NWTile<num_nb, num_ic, num_oc>::setID(UI id)
{
    ...
    case MESH3D:
		// North border
		if(SurfaceborderN(id)) 	portN = ND;	
		else 			portN = 0;

		// South border
		if(SurfaceborderS(id)) 		portS = ND;
		else if(SurfaceborderN(id)) 	portS = 0;	
		else 				portS = 1;

		// East border
		if(SurfaceborderE(id)) 		portE = ND;	
		else if(SurfaceborderN(id) || SurfaceborderS(id)) portE = 1;	
		else 				portE = 2;		

		//West border
		if(SurfaceborderW(id)) 					portW = ND;
		else if(corner3D(id)|| EdgeNE(id) || EdgeSE(id))	portW = 1;
		else if(SurfaceborderN(id) || SurfaceborderS(id))	portW = 2;
		else if(SurfaceborderE(id)) 				portW = 2;
		else if(SurfaceborderF(id) || SurfaceborderR(id))	portW = 3;
		else 							portW = 3;
		
		//Front border
		if(SurfaceborderF(id)) 			portF = ND;		
		else if(corner3D(id)) 			portF = 2;		
		else if(EdgeNR(id) || EdgeSR(id) || EdgeRE(id) || EdgeRW(id))	portF = 3;
		else if(Edge3D(id))			portF = 2;		
		else if(SurfaceborderR(id))		portF = 4;		
		else if(Surfaceborder3D(id))		portF = 3;		
		else 					portF = 4;		
		
		//Rear border
		if(SurfaceborderR(id)) 			portR = ND;		
		else if(corner3D(id)) 			portR = 2;		
		else if(Edge3D(id))			portR = 3;		
		else if(Surfaceborder3D(id))		portR = 4;		
		else 					portR = 5;		
		break;
    }
	if(TOPO == MESH3D)
	{
		...
		for(int i = 0; i < num_ic; i++)
			Ichannel[i]->setTileID(id, portN, portS, portE, portW, portF, portR);

		// set id and port mapping for OCs
		for(int i = 0; i < num_oc; i++)
			Ochannel[i]->setTileID(id, portN, portS, portE, portW, portF, portR);

		// set id and port mapping for VCA
		vcAlloc.setTileID(id, portN, portS, portE, portW, portF, portR);
		...
		ctr.setTileID(id, portN, portS, portE, portW, portF, portR);
	}
	else
	{
	    // set id and port mapping for ICs
...
void NWTile<num_nb, num_ic, num_oc>::calc_results()
{
    ...
    LinkUsage= rs.total_ic_pktin;		// Counting number of links used to transmit the flits
    if (RT_ALGO == QRT)
    {
...
/////////////// 
//	returns total throughput
//	////////////

template <int num_nb, int num_ic, int num_oc>
 float NWTile<num_nb, num_ic, num_oc>::return_total_throughput() {
	float total_throughput = 0;
	for(int i = 0; i < num_oc; i++) {
		total_throughput += Ochannel[i]->avg_throughput;
	}
	//cout<<"\n  622 NWTile.cpp tile-"<<tileID<<"  total_tput="<<total_throughput<<"   num_oc="<<num_oc<<"  Avg_tput ="<<total_throughput;
	//return total_throughput/num_oc;
	return total_throughput;	// in output channel avg_throughput is already divided by number of channels used.
}
...
/////////////////////////////////////////////////////////////////////
/// Returns id corresponding to a given port direction (N, S, E, W, F, R)
////////////////////////////////////////////////////////////////////
template <int num_nb, int num_ic, int num_oc>
...
template struct NWTile<NUM3D_NB, NUM3D_IC, NUM3D_OC>;
template struct NWTile<NUM3D_NB_SB, NUM3D_IC_SB, NUM3D_OC_SB>;
// template struct NWTile<NUM3D_NB_C, NUM3D_IC_C, NUM3D_OC_C>;
// template struct NWTile<NUM3D_NB_EB, NUM3D_IC_EB, NUM3D_OC_EB>;
